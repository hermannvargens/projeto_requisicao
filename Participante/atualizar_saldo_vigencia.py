...
def atualizar_saldo_vigencia(nome_arquivo_csv):
    ...
    df_itens['Início da Vigência'] = df_itens['Início da Vigência'].astype(str)
    df_itens['Fim da Vigência'] = df_itens['Fim da Vigência'].astype(str)
    df_itens['Qtd. Saldo'] = df_itens['Qtd. Saldo'].astype(str)
    ...
    for idx, row in df_itens.iterrows():
        ...
        print('Iniciando atualização do saldo, início da vigência e fim da vigência...')
